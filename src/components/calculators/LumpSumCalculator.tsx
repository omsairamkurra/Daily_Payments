...
export default function LumpSumCalculator() {
  ...
  const results = useMemo(() => {
    const maturityValue = calculateLumpSum(investmentAmount, years, returnPercent)
    const totalReturns = maturityValue - investmentAmount
    return { maturityValue, totalReturns }
  }, [investmentAmount, years, returnPercent])

  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {/* Inputs */}
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">Lump Sum Calculator</h3>
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-xl p-4">
            <p className="text-sm text-blue-600 dark:text-blue-400 font-medium">Invested Amount</p>
        ...
        <ComparisonChart
          sipMonthly={Math.round(investmentAmount / (years * 12))}
          lumpSumAmount={investmentAmount}
          years={years}
